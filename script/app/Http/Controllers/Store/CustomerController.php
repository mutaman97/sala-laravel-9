...
class CustomerController extends Controller
{
    ...
    public function change_password()
    {
        abort_if(tenant('customer_modules') != 'on',404);
        $meta=json_decode(Auth::user()->meta ?? '');
        SEOMeta::setTitle('Password Settings');
        return view(baseview('customer/password_settings'),compact('meta'));
    }
    ...
    public function profilePasswordUpdate(Request $request)
    {
        ...
        if ($check==true) {
            ...
            $error['errors']['domain']=__('Enter a valid password');
    ...
    public function makeReview(Request $request,$orderid,$termid,$optionid)
    {
        ...
        if (!empty($review)) {
            ...
            $newAverageRating = Review::where('term_id', $termid)->avg('rating');

            if ($previousRating != $request->rating) {
                $term = Term::findOrFail($termid);
                $term->rating = $newAverageRating;
                $term->save();
            }

            return response()->json(__('Review Updated Successfully'));
        } else {
            $review = new Review;
            ...
            return response()->json(__('Review Added Successfully'));
        }
