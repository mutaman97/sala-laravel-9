...
Route::group(['middleware' => ['Isinstalled','InitializeTenancyByDomain','PreventAccessFromCentralDomains','web','tenantenvironment','CheckTenantForMaintenanceMode']], function () {
    ...
    Route::get('/category/{slug}', 'Store\PageController@category')->name('category');
    ...
    Route::get('blog', 'Store\BlogController@index')->name('blog.index');
    Route::get('blog/{slug}', 'Store\BlogController@show')->name('blog.show');
    ...
    // added by mutaman to remove coupon code
    Route::post('removediscount','Store\CartController@removediscount')->name('removediscount');
    // end
    ...
    Route::group(['prefix'=>'customer'],function(){
        ...
        Route::get('/change-password', 'Store\CustomerController@change_password')->middleware(['auth','user']);
